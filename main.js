(() => {
  ...
  if (!window.THREE) {
    document.getElementById("loading").innerHTML =
      "<div><h1>Erro</h1><p>Three.js não carregou. Confira sua internet.</p></div>";
    return;
  }
  ...
  const QUALITY = [
    { name: "Ultra leve", pixel: 0.45, buildings: 28, trees: 55, clouds: 4, fog: 4200, particles: 24 },
    { name: "Baixa", pixel: 0.62, buildings: 50, trees: 100, clouds: 7, fog: 5600, particles: 36 },
    { name: "Alta", pixel: 0.88, buildings: 95, trees: 180, clouds: 14, fog: 8200, particles: 58 }
  ];
  ...
  scene.add(new THREE.HemisphereLight(0xffffff, 0x397739, 2.05));
  ...
  function createMountains() {
    ...
    const mesh = new THREE.InstancedMesh(geometry, material, 24);
    ...
    for (let i = 0; i < 24; i++) {
      ...
      dummy.position.set(side * (1700 + Math.random() * 1100), h / 2, -1900 + Math.random() * 1500);
  ...
  function rebuildDynamicWorld() {
    ...
    dynamicObjects.push(createBuildingWindows(Math.floor(quality.buildings * 4)));
  ...
  function createBuildingWindows(count) {
    ...
    for (let i = 0; i < count; i++) {
      const x = (Math.random() - 0.5) * 1600 + 1050;
      const z = (Math.random() - 0.5) * 1850 - 545;
  ...
  function createClouds(count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let j = 0; j < puffs; j++) {
        ...
        puff.scale.set(55 + Math.random() * 95, 13 + Math.random() * 13, 35 + Math.random() * 65);
        puff.position.set((j - puffs / 2) * 45, Math.random() * 16, Math.random() * 24);
        cloud.add(puff);
      }

      cloud.position.set((Math.random() - 0.5) * 6800, 560 + Math.random() * 850, (Math.random() - 0.5) * 6800);
  ...
  function createPlane() {
    ...
    const glassMaterial = new THREE.MeshBasicMaterial({ color: 0x174a78, transparent: true, opacity: 0.75 });
  ...
  function updatePhysics(dt) {
    ...
    if (down("s")) aircraft.throttle -= 0.75 * dt;
    ...
    // Motor corrigido: acelera de forma clara e jogável.
    const thrustAccel = aircraft.throttle * 34; // m/s² arcade-realista
    aircraft.velocity.addScaledVector(fwd, thrustAccel * dt);

    // Arrasto: cresce com a velocidade e impede velocidade infinita.
    const drag = 0.0018 * speed * speed + 0.025 * speed;
    if (speed > 0.01) {
    ...
    // Sustentação: velocidade + pitch + ângulo de ataque.
    ...
    aircraft.stall = horizontalSpeed > 25 && absAoa > aircraft.stallAngle;

    let liftFactor = clamp((horizontalSpeed - 24) / 58, 0, 1.65);
    let aoaLift = clamp(0.35 + aircraft.pitch * 1.75 + aoa * 1.35, -0.35, 1.65);

    if (aircraft.stall) {
      aoaLift *= 0.25;
    }

    const liftAccel = liftFactor * aoaLift * 13.5;
    aircraft.velocity.y += liftAccel * dt;

    // Controles
    const authority = clamp(horizontalSpeed / 45, 0.25, 1.45);

    aircraft.angularPitch += elevator * 1.55 * authority * dt;
    aircraft.angularRoll += aileron * 2.35 * authority * dt;
    aircraft.angularYaw += rudder * 0.95 * authority * dt;

    // Curva coordenada: roll faz o avião virar
    aircraft.angularYaw += Math.sin(aircraft.roll) * 0.42 * authority * dt;

    // Stall: nariz cai e balança.
    if (aircraft.stall) {
      aircraft.angularPitch -= 1.1 * dt;
      aircraft.angularRoll += Math.sin(performance.now() * 0.006) * 1.15 * dt;
      aircraft.angularYaw += Math.cos(performance.now() * 0.005) * 0.5 * dt;
    }

    // Damping
    aircraft.angularPitch *= Math.pow(0.18, dt);
    aircraft.angularRoll *= Math.pow(0.14, dt);
    aircraft.angularYaw *= Math.pow(0.24, dt);

    aircraft.pitch += aircraft.angularPitch * dt;
    aircraft.roll += aircraft.angularRoll * dt;
    aircraft.yaw += aircraft.angularYaw * dt;

    aircraft.pitch = clamp(aircraft.pitch, -0.85, 0.85);
    aircraft.roll = clamp(aircraft.roll, -1.45, 1.45);

    aircraft.position.addScaledVector(aircraft.velocity, dt);

    handleGroundCollision(horizontalSpeed);
    syncPlane();
  }

  function handleGroundCollision(horizontalSpeed) {
    ...
    const hardHit = verticalImpact > 7.5 || bank > 0.85 || nose > 0.65 || speedKmh > 185;

    if (hardHit) {
      const damage = Math.round(verticalImpact * 10 + speedKmh * 0.25 + bank * 35 + nose * 35);
      aircraft.health -= damage;

      if (aircraft.health <= 0 || verticalImpact > 12 || speedKmh > 230) {
        explodePlane();
      } else {
    ...
    // Atrito de pista, mas sem matar aceleração.
    const friction = aircraft.throttle > 0.05 ? 0.992 : 0.965;
    aircraft.velocity.x *= friction;
    aircraft.velocity.z *= friction;

    // No chão, o avião estabiliza.
    aircraft.pitch *= 0.985;
    aircraft.roll *= 0.94;

    // Direção no solo com A/D.
    const rudder = (down("a") ? 1 : 0) - (down("d") ? 1 : 0);
    aircraft.yaw += rudder * clamp(horizontalSpeed / 30, 0.15, 1) * 0.025;
  }
  ...
  function syncPlane() {
    ...
    if (plane.userData.prop) {
      plane.userData.prop.rotation.z += 0.2 + aircraft.throttle * 1.5;
    }
  ...
  function updateCamera(dt) {
    ...
    const desired = plane.visible
      ...
      : aircraft.position.clone().add(new THREE.Vector3(0, 70, 90));
